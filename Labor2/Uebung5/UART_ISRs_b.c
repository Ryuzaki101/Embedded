...
void UART0_ISR (void) __irq 
{
  ...
  while (((IIR_Content = U0IIR) & 0x1) == 0  ) {
	...
	if ( IIR_Content & 0x100){  //Check if auto-baud interrupt flag is set
		AutoBaud_U0DLL = U0DLL;
		AutoBaud_U0DLM = U0DLM;
		U0ACR |= (IIR_Content & 0x300); // Clear both auto-baud interrupt flags
	}
	else
		if ( IIR_Content & 0x200)  //Check if auto-baud time-out interrupt flag is set
					U0ACR |= (IIR_Content & 0x200); // Clear auto-baud time-out interrupt flags
 }
 VICVectAddr = 0;
}	  
...
void UART1_ISR (void) __irq 
{
  ...
  while (((IIR_Content = U1IIR) & 0x1) == 0  ) {
	...
	if ( IIR_Content & 0x10){
		...
		U1IIR = (IIR_Content & 0x30);
	}
	else
		if ( IIR_Content & 0x20)
					U0IIR = (IIR_Content & 0x20);
 }
 VICVectAddr = 0;
}	  
...
void EXTINT0_Launch_ISR (void) __irq{
  ...
  EXTINT      |= 1 << 0;                            // Clear EINT1 interrupt flag
...
void  init_Timer0_for_UART(void) {
	...
	T0PR  = 0x00;		/* set timer 0 prescaler to zero */
	T0CTCR = 0x0;       /* Counter based upon the internal clock */
	T0MR0 = 2 * 14400000;  // Voraussetzung: Timer mit einer Taktfrequenz von 14,4 MHz
	T0IR  = 0xff;		/* reset all interrrupts */
	T0MCR = 0x03;		/* Interrupt and reset timer on match */
