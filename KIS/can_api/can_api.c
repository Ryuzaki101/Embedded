...
/*********************************************************************************
		Declaration of CAN-related constants
**********************************************************************************/
...
const uint16_t  CAN1_Array_SFF_EXP_IDs[CAN1_Number_SFF_EXP_ID] = {0x456, 0x654};
...
const uint16_t  CAN2_Array_SFF_EXP_IDs[CAN2_Number_SFF_EXP_ID] = {0x333,0x444};
  ...
  if (( CAN_Module_Number == 1) || ( CAN_Module_Number == 2))
   {
    ...
    CAN_Regs_ptr->IER = 1;  // Disable Receive Interrupts
	CAN_Init_Done = True;
   }
...
{		
  ...
  if  ((CANCR->CANRxSR) & (1 << 8))
         CAN_Receive_Message(1, &Received_Message);
  if  ((CANCR->CANRxSR) & (1 << 9))
         CAN_Receive_Message(2, &Received_Message);
  if ( (CAN1->GSR) & (1 << 6 ) )
  {
